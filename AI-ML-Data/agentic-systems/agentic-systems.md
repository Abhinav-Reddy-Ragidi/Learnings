...
## 4. The guiding agent today

![One turn, and where each of the eight model calls sits](diagrams/guide-turn.pdf){ width=100% }

- **Tasks it has**: 18 of them, listed and typed in §7.1 — answering with traceable citations, reading the team's repo, uploads, web and papers, maintaining milestones and the journal, recording verification verdicts, drawing artifacts.
- **Capabilities**: 22 tools across 10 families (§6), every read and write tenant-scoped through apps/api; no execution environment — a verdict rests on what was read, never on what was run.
...
- **Missing**: the benchmark. No graded set of real turns exists, so no change to prompt, model or tools is verifiable (§9, G-03).


\newpage

## 5. Agent registry — what goes in, what must come out
...
### 5.1 What each one is actually given
...
### 5.2 Two properties that cut across the table
...
### 5.3 Rules the table cannot carry
...
## 6. Tool catalogue — the 22 tools the guide agent has
...
## 7. Tasks, and whether one loop fits them

### 7.1 Task inventory

Every task the guiding agent is expected to handle, with the kind of task it is. The type column is what decides the architecture — §7.2 works from it.
...
### 7.2 Does one agentic loop fit these tasks?

![Which task types the loop holds, and which outgrow it](diagrams/task-fit.pdf){ width=90% }

#### 7.2.1 The task types, and the property that matters
...
#### 7.2.2 Where one loop is the right shape
...
#### 7.2.3 Where it strains, in order of how much it hurts
...
### 8.4 Drill — when a bad answer is reported
...
4. Tag it (§8.1).
5. Assign one root cause (§8.2).
6. Add the case (§8.3), with the answer it should have given.
7. Fix, replay the whole table, record which change fixed it.


## 9. Issue and fix register
...
| G-03 | No benchmark | no change to prompt, model or tools is verifiable | build §8.3 from ~30 real turns before the next change | open |
| G-04 | The outbound reviewer refuses good answers | it judges every number while blind to what the tools read (§5.2), so a figure read off a screenshot looks identical to an invented one | pass a record of the work performed — tools run and what they returned — into the reviewer | in progress |
...
## 10. One turn, end to end — the implementation walkthrough

Written so a developer can follow one student message through all three processes without opening the code. Everything below is what the code does today, not what the design documents say it does. Section 4 is the shape; this is the wiring.
...
### 10.1 Before the message — the upload path
...
### 10.3 Four gates before anything is written
...
### 10.4 The transaction — three rows
...
### 10.9 What the model is actually given
...
### 10.10 What a tool returns — the fork that decides traceability
...
### 10.16 `completeTurn` — what apps/api does with it
...
### 10.19 Where to look when it goes wrong
